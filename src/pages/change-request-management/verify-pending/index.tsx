...
import { useVerifyPendingChanges, type ChangeRequest } from "../queries";
...
export default function ChangeRequestVerifyPending() {
  const navigate = useNavigate();
  const { data, isPending } = useVerifyPendingChanges();
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <DataTable
        ...
        data={rows}
        emptyMessage={
          isPending ? "불러오는 중…" : "검증 대기중인 변경이 없습니다."
        }
        onRowClick={(row) =>
          navigate(
            `/configuration-changes/detail/${row.id}?from=/configuration-changes/pending-verify`
          )
        }
      />
    </div>
